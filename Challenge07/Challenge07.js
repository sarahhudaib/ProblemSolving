...
const objLat = (obj) => {
    return `my name is ${obj.firstName.charAt(0).toUpperCase() + 
        obj.firstName.substring(1)} ${obj.lastName.charAt(0).toUpperCase() + 
            obj.lastName.substring(1)} I am ${obj.age} YO, and I love ${obj.hobby}.`
};
// const objLat = (obj) => {
...
const cvFormatter = (arr) => {
    ...
    let arr3 = function (arr2) {
        return arr2.map(function (opt) {
            let answer = {};
            if (opt.firstName == null) {
                answer['fullName'] = opt.lastName;
                answer['tech'] = opt.tech;
            } else if (opt.lastName == null) {
                answer['fullName'] = opt.firstName;
                answer['tech'] = opt.tech;
            } else {
                answer['fullName'] = `${opt.firstName} ${opt.lastName}`;
                answer['tech'] = opt.tech;
            }
...
const applicationsStatics = (arr) => {
    let answer = {
        ...
        rejectedApplicants: 0,}
    for (let i = 0; i < arr.length; i++) {
        if  ((arr[i].firstName == '' || arr[i].firstName == null) && (arr[i].lastName == '' || arr[i].lastName == null)){
            answer.rejectedApplicants++
            continue}
            if (arr[i].yearsOfExperience <= 1) {
                answer.rejectedApplicants++
                continue
    ...
    for (let i = 0; i < arr.length; i++) {
        switch (arr[i].tech) {
            case "JS": answer.javaScript_Devs++
                break;
            case ".Net": answer.dotNet_Devs++
                break;
            case "Python": answer.python_Devs++
                break;
            case "Java": answer.java_Devs++
        }}
        answer.totalApplicants = arr.length
    return answer;   
};
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
      for (let j = 0; j < data.grades[i].numberOFClasses; j++) {
        count = data.grades[i].classes[j].classScores.reduce(function (a, b) {
          return a + b;
        }, 0);
        data.grades[i].classes[j].avg = Math.floor(count / data.grades[i].classes[j].classScores.length);
      }count = 0;}return data;
    };
